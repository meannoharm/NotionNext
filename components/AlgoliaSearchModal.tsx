...
export default function AlgoliaSearchModal() {
  ...
  const handleSearch = async (query: string, page: number) => {
    ...
    try {
      ...
      const res = await algoliaRef.current.searchSingleIndex<AlgoliaRecord>({
  ...
  return (
    ...
    >
      {/* 模态框 */}
      <div
        className={`${isModalOpen ? 'opacity-100' : 'invisible translate-y-10 opacity-0'} dark:bg- dark:bg-hexo-black-gray z-50 flex min-h-[10rem] w-full max-w-xl flex-col justify-between rounded-lg border bg-white p-5 shadow transition-all duration-300 hover:border-blue-600 dark:border-gray-800 `}
      >
        ...
        <div>
          {totalHit > 0 && (
            <div>{t('search_result', { totalHit, useTime })}</div>
          )}
        </div>
